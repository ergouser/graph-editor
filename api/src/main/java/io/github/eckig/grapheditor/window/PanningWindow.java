...
public class PanningWindow extends Region {
   ...
   *
   * <p>
   * This method is called by the framework. Custom skins should <b>not</b> call it. Editor properties should instead be
   * set via the graph editor instance.
   * </p>
   *
   ...
   *
   * @param x
   *          the x position of the window relative to the top-left corner of the content
   * @param y
   *          the y position of the window relative to the top-left corner of the content
   */
   ...
   * <p>
   * <b>Note: </b><br>
   * The current width and height values of the window and its content are used in this method. It should therefore be
   * called <em>after</em> the scene has been drawn.
   * </p>
   *
   * @param position
   *          the {@link Pos} to pan to
   */
  ...
  protected void handleZoom(final ZoomEvent pEvent) {
    ...
    if (pEvent.getEventType() == ZoomEvent.ZOOM_STARTED
        && graphEditorProperties.activateGesture(GraphInputGesture.ZOOM, pEvent, this)) {
      pEvent.consume();
