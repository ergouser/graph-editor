...
class MinimapNodeGroup extends Parent {
  ...
  /**
   * Sets the GraphEditor to resolve skins for the nodes drawn in the minimap.
   *
   * @param pModel
   *          the {@link GModel} whose nodes are to be drawn
   */
  ...
  /**
   * Set a filter {@link Predicate} to only draw the desired connections onto the minimap
   *
  ...
  /**
   * @return {@link ObjectProperty} controlling the {@link Color} to paint the connections onto the minimap
   */
  ...
  /**
   * @param pScaleFactor
   *          the ratio between the size of the content and the size of the minimap (between 0 and 1)
   */
...
public void drawOnFXThread() {
  ...
  protected void layoutChildren() {
    ...
    if (model != null) {
      try {
        for (GConnection connection : model.getConnections()) {
          ...
          if ( source != null ) {
            ...
            if ( parentSource != null ) {
              ...
              GConnectorSkin sourceSkin = skinLookup.lookupConnector(source);
              GNodeSkin parentSkin = skinLookup.lookupNode(parentSource);
              if (sourceSkin != null && parentSkin != null ) {
                double x = scaleSharp(sourceSkin.getX() + parentSkin.getX() - 10, scaleFactor),
        ...
        e.printStackTrace(); // don't allow minimap to crash the app if something goes wrong with the skin lookup or drawing.
      }
    }
  }
  ...
  private <N extends Node> void resizeRelocate(final GNode node, final Node rendered,
    ...
    final double x = (Math.round(nodeSkin.getX() * scaleFactor));
    final double y = (Math.round(nodeSkin.getY() * scaleFactor));
    final double width = (Math.round(nodeSkin.getWidth() * scaleFactor));
    final double height = (Math.round(nodeSkin.getHeight() * scaleFactor));
