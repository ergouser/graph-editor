...
public class GraphEditorMinimap extends PanningWindowMinimap {
  ...
  protected static final double INITIAL_ASPECT_RATIO = 0.75;

  // Minimap height is not specified here, the minimap's aspect ratio is fixed by the aspect ratio of the content.
  protected static final double MINIMAP_WIDTH = 250;

  protected final MinimapNodeGroup minimapNodeGroup = new MinimapNodeGroup();

  protected GModel model;

  protected final CommandStackListener modelChangeListener = event -> minimapNodeGroup.draw();
  ...
  protected double computePrefHeight(double width) {
    ...
    final double contentRatio = getContent() == null ? INITIAL_ASPECT_RATIO
  ...
  public void setModel(final GModel pModel) {
    ...
    if (pModel != null) {
      CommandStack.getCommandStack(pModel).addCommandStackListener(modelChangeListener);
