 ...
 * </p>
 */
public abstract class ResizableBox extends DraggableBox {
  ...
  private static boolean isResizeCursor(final Cursor pCursor) {
    return pCursor == Cursor.NE_RESIZE || pCursor == Cursor.NW_RESIZE || pCursor == Cursor.SE_RESIZE
        || pCursor == Cursor.SW_RESIZE || pCursor == Cursor.N_RESIZE || pCursor == Cursor.S_RESIZE
        || pCursor == Cursor.E_RESIZE || pCursor == Cursor.W_RESIZE;
  }
   ...
   *
   * @param pEvent
   *          the latest {@link MouseEvent} for the mouse entering or moving inside the rectangle
   */
   ...
   *
   * @param mouseRegion
   *          the {@link RectangleMouseRegion} where the cursor is located
   */
